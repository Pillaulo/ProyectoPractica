...
import { type GroqProvider } from '../providers/groq/groqProvider.js';
import { type StoryRequest, type StoryResponse, type StorySessionDetail, type StorySessionListItem } from '../types/story.js';
import { type StorySessionRepository } from '../repositories/storySessionRepository.js';
...
function buildPrompt(input: StoryRequest): { system: string; user: string } {
  const system = [
...
function safeParseStoryJson(content: string): StoryResponse | null {
  try {
...
export class StoryService {
  constructor(
    ...
    private readonly repo: StorySessionRepository
  ) {}

  async generateAndSaveStory(input: StoryRequest): Promise<StoryResponse> {
    const { system, user } = buildPrompt(input);

    const attempt = async (): Promise<StoryResponse | null> => {
  ...
  async listSessions(): Promise<StorySessionListItem[]> {
    try {
  ...
  async getSession(id: number): Promise<StorySessionDetail> {
